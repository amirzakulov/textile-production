<template>
    <Modal
        v-model="productionStore.fpDetailsAddOutModal"
    ...
    >
        ...
        <Form ref="formValidate" :label-width="180">
            <FormItem label="* Махсулот" class="ivu-mb">
                <Select v-model="productionStore.departmentFinishProduct.raw_material_balance_id"
                        @on-change="productChanged(productionStore.departmentFinishProduct.raw_material_balance_id)"
                        filterable placeholder="Махсулотни танланг...">
                    <template v-for="rmbProduct in productionStore.departmentFinishProducts" :key="rmbProduct.id">
            ...
            <FormItem label="* Миқдор" class="ivu-mb">
                <Input v-model="productionStore.departmentFinishProduct.count" @on-change="countChanged" placeholder="Махсулот Миқдор"></Input>
        ...
        <template #footer>
            <Button @click="productionStore.fpDetailsAddOutModal = false">Беркитиш</Button>
...
export default {
    ...
    methods: {
        async addProductionRmProductOut(){
            this.isAdding = true
            if(this.productionStore.departmentFinishProduct.raw_material_balance_id == null) {
            ...
            if(this.productionStore.departmentFinishProduct.count === ""
                || this.productionStore.departmentFinishProduct.count === 0
                || this.productionStore.departmentFinishProduct.count === undefined
            ) {
                this.is_invalid = true
                this.raw_material_count_err.display   = true
                this.raw_material_count_err.message   = "Тўлдириш мажбурий"
            } else if(this.productionStore.departmentFinishProduct.count > this.stockCount) {
                this.is_invalid = true
                this.raw_material_count_err.display   = true
            ...
            let outgoingProduct = {
                ...
                rawMaterial:    this.productionStore.departmentFinishProduct,
                department_id:  this.productionStore.department_id
            }

            const res = await this.callApi('post', '/app/add_production_finish_product_out', outgoingProduct)
            ...
            if(res.status === 200) {
                if(res.data === false){
                    ...
                    this.productionStore.inoutSetRawMaterialsId.push(this.productionStore.departmentFinishProduct.raw_material_id)
                    this.productionStore.departmentRawMaterial = {}
                }

                this.productionStore.fpDetailsAddOutModal   = false
