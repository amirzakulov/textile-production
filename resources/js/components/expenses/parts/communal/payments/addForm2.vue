<template>
    <Form label-position="top">
        ...
        <Row :gutter="8">
            ...
            <Col span="5">
                <FormItem label="Охирги кўрсаткич (реактив)">
                    <Input v-model="data.reactive_value"></Input>
                    <Text :class="{ 'd-none': !payment_reactive_value_err.status }" type="danger" class="m-0 p-0">{{payment_reactive_value_err.message}}</Text>
                </FormItem>
            </Col>
            <Col span="5">
                <FormItem :label="`Нарх (реактив, 1 ${communalTypesStore.communalTypeData.measurement})`">
                    <Input v-model="data.reactive_price"></Input>
...
export default {
    ...
    data(){
        return {
            ...
            payment_reactive_value_err: {status: false, message:''},
    ...
    methods: {
        async addPayment(){
            ...
            if(this.data.reactive_value == '') {
                this.is_invalid                         = true
                this.payment_reactive_value_err.status  = true
                this.payment_reactive_value_err.message = 'Тўлдириш мажбурий'
            } else {
                this.payment_reactive_value_err.status = false
            }

            if(this.data.reactive_price == '') {
                this.is_invalid         = true
            ...
            if(Object.keys(this.communalTypesStore.lastPayment).length > 0) {
                ...
                this.data.reactive_amount = this.data.reactive_value - this.communalTypesStore.lastPayment.reactive_value
            } else {
                this.data.active_amount = 0
                this.data.reactive_amount = 0
            }
            ...
            if(res.status === 200) {
                ...
                this.data.reactive_value = ''
                this.data.reactive_price = ''
                this.data.payment       = ''
                this.data.date          = ''
