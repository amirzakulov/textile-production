<template>
    <Modal
        v-model="rmStore.productAddModal"
    ...
    >
        ...
        <template #footer>
            <Button @click="rmStore.productAddModal = false">Беркитиш</Button>
            <Button type="primary" @click="addRmProduct" :disabled="isAdding" :loading="isAdding">{{isAdding ? 'Махсулот...':'Сақлаш'}}</Button>
        </template>
    </Modal>
</template>
...
export default {
    ...
    methods: {
        async addRmProduct(){ //Add tag
            ...
            this.rmStore.product.picture        = null
            this.rmStore.product.fashion_id     = null
            this.rmStore.product.department_id  = this.rmStore.department_id
            this.rmStore.product.isRawMaterial  = 1

            const res = await this.callApi('post', '/app/add_product', this.rmStore.product)

            if(res.status === 200) {
                this.isAdding = false
                this.rmStore.products.unshift(res.data);
                this.rmStore.productAddModal = false
